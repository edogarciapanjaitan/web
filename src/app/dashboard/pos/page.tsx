...
export default async function PosPage() {
  ...
  // We can just calculate daily totals from history API
  const { fetchDailyHistory } = await import("../history/history-actions");
  const historyResponse = await fetchDailyHistory(dateStr);
  const dailyTransactions = historyResponse.data || [];
  
  const dailyCashSales = dailyTransactions
    .filter((tx) => tx.paymentMethod === "CASH")
    .reduce((sum, tx) => sum + tx.totalPrice, 0);

  const dailyDebitSales = dailyTransactions
    .filter((tx) => tx.paymentMethod === "DEBIT")
    .reduce((sum, tx) => sum + tx.totalPrice, 0);

  return (
    <div className="min-h-screen flex flex-col py-6 px-8 max-w-350 mx-auto w-full">
      <div className="flex items-center gap-4 mb-6">
        <NavConfirmLink
          href="/dashboard"
          message="Anda akan keluar dari halaman POS dan kembali ke Dashboard. Lanjutkan?"
          className="flex items-center gap-1.5 text-(--text-secondary)] no-underline text-sm font-medium py-1.5 px-3 rounded-lg transition-all duration-200 hover:text-(--foreground)] hover:bg-[rgba(255,255,255,0.06)]"
        >
      ...
      <PosClient 
        ...
        initialDailyCashSales={dailyCashSales}
        initialDailyDebitSales={dailyDebitSales}
      />
    </div>
  );
}
