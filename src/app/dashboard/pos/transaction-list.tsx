  ...
  return (
    <div className="bg-(--card-bg)] border border-(--card-border)] rounded-2xl backdrop-blur-lg">
      <div className="flex items-center gap-2 py-4 px-5 border-b border-(--card-border)] text-(--foreground)]">
        ...
        <h3 className="text-base font-semibold m-0 flex-1">Transaksi Shift Ini</h3>
        <span className="text-xs text-(--text-muted)] bg-[rgba(255,255,255,0.06)] py-0.5 px-2 rounded-full font-medium">{transactions.length}</span>
      </div>
        ...
        {transactions.map((trx) => (
          <div key={trx.id} className="py-3 px-5 border-b border-[rgba(255,255,255,0.04)] flex flex-col gap-1 transition-colors duration-150 last:border-b-0 hover:bg-[rgba(255,255,255,0.02)]">
            <div className="flex justify-between items-center">
              ...
              <span className="text-xs text-(--text-muted)]">{formatTime(trx.createdAt)}</span>
            </div>
            <div className="flex justify-between items-center">
